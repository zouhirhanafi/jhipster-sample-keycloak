...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: AlertService, private eventManager: EventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('jhipsterSampleKeycloakApp.error', (response: EventWithContent<unknown> | string) => {
            ...
            if (errorHeader) {
              ...
              for (const fieldError of fieldErrors) {
                ...
                const fieldName: string = translateService.instant(
                  `jhipsterSampleKeycloakApp.${fieldError.objectName as string}.${convertedField}`
                );
